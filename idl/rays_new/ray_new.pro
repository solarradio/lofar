pro ray_new, eps=eps, anis=anis, R_init=R_init, asym=asym, f_ratio=f_ratio
; +
...
while (N_elements(r) GE N/200) DO begin  
 ...
 omega=sqrt(omega_pe(sqrt(rx*rx+ry*ry+rz*rz))^2+kc*kc)
  ...
  dt=min([.1D0/max(nu_s),dt0,dt_ref,dt_dr])
  ;dt=dt0/5
...
IF N_elements(where(r GE r_stop,/NULL)) GE 1 THEN BEGIN
  ...
  IF (t GE tplot+dT_save) THEN BEGIN
    ...
    ;wset, 0
    plot,[rz0],[rx0],psym=2,xrange=[-r_init,R_stop],yrange=[-1.,1.]*(r_init+r_stop)/2, xtitle='Z [R!D!9n!3!N]',ytitle='X [R!D!9n!3!N]'
    pp = findgen(41)/40*!pi*2
    oplot,cos(pp), sin(pp),thick=2,color=5
    oplot,r_stop*cos(pp), r_stop*sin(pp),line=1
    oplot,[0,0],[R_stop,-R_stop],line=1
...
print, 'Tmax=', T_max, ' Delay =',t_max, '  HM before =',t_HM_before, '   =',t_HM_after
...
print, 'Predicted FWHM source sizes [arcmin] =',atan((rint[mloc1]-R_init)*0.5/(au-r_init))*360./!PI
...
function density_r, rr
  ...
  return, 4.8e9/RR^14.+ 3e8/RR^6.+1.39d6/RR^2.3+nc
  ;+nc
